...
private:
    ...
    MutableContainer writingContainer_{};
    internal_file_name_t writingContainerName_{};
    std::size_t writingContainerOffset_{};
    std::mutex writingContainerMtx_{};
    ...
    void createWritingContainer_() {
        writingContainerName_ = containerNameDispenser_.get();
        writingContainer_.create(writingContainerName_);
        writingContainerOffset_ = 0;
    }

public:
    BackendFacade() {
        createWritingContainer_();
    }
    ...
    void finishRecipeFile(const user_id_t &userID, const fileShareMetaHead_t &fileShareMetaHead, const key_t &key) {
        ...
        if (findIter->second.second == fileRecipeHead.numOfShares) {
            ...
            DataBase::Put(key, {recipeFileBuffer.get(), kRecipeSize});
            {
   ...
   */
    void updateShareIndex(const user_id_t &userID, const key_t &key, std::string &value) {
        ...
        // get the head and the appending entry
    ...
    std::pair<internal_file_name_t, std::size_t> putShareData(bytes_view shareData) {
        std::lock_guard<decltype(writingContainerMtx_)> lockGuard{writingContainerMtx_};
        benchmark::ScopedLap lap{Benchmark::DiskWriteTimer()};
        if (writingContainerOffset_ + shareData.size() > writingContainer_.size()) {
            // this container file is full, open a new container
            createWritingContainer_();
        }
        std::copy(shareData.cbegin(), shareData.cend(), writingContainer_.region().begin() + writingContainerOffset_);
        auto off = writingContainerOffset_;
        writingContainerOffset_ += shareData.size();
        return {writingContainerName_, off};
    };

    /**
      * @brief get the data corresponding to the given key, and the data type depends on the prefix of the key
      ...
      */
    std::optional<std::string> get(const key_t &key) {
        if (key[0] == static_cast<std::byte>(IndexPrefix::RECIPE)) {
    ...
    /**
     * @brief get the share data from container
